...
{
    public struct EcsQuery<T1, T2, T3, T4, T5, T6, T7, T8>
        where T1 : struct
        where T2 : struct
        where T3 : struct
        where T4 : struct
        ...
        where T8 : struct
    {
        ...
        {
            return new Enumerator(_pool1, _pool2, _pool3, _pool4, _pool5, _pool6, _pool7, _pool8, _primaryEntities, _primaryCount, _primaryIndex);
        }
                ...
                while (++_index < _primaryCount)
                {
                    ...
                    if (_primaryIndex == 1)
                    {
                        if (_pool2.Has(entity) && _pool3.Has(entity) && _pool4.Has(entity) && _pool5.Has(entity)
                            && _pool6.Has(entity) && _pool7.Has(entity) && _pool8.Has(entity))
                        {
                            _current = entity;
                            return true;
                        }
                    ...
                    {
                        if (_pool1.Has(entity) && _pool2.Has(entity) && _pool3.Has(entity) && _pool4.Has(entity)
                            && _pool5.Has(entity) && _pool7.Has(entity) && _pool8.Has(entity))
                        {
                            _current = entity;
                            return true;
                        }
                    ...
                    {
                        if (_pool1.Has(entity) && _pool2.Has(entity) && _pool3.Has(entity) && _pool4.Has(entity)
                            && _pool5.Has(entity) && _pool6.Has(entity) && _pool8.Has(entity))
                        {
                            _current = entity;
                            return true;
                        }
                    ...
                    {
                        if (_pool1.Has(entity) && _pool2.Has(entity) && _pool3.Has(entity) && _pool4.Has(entity)
                            && _pool5.Has(entity) && _pool6.Has(entity) && _pool7.Has(entity))
                        {
                            _current = entity;
                            return true;
                        }
