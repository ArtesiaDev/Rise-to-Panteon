...
{
    public struct EcsQuery<T1, T2, T3, T4>
        where T1 : struct
        where T2 : struct
        where T3 : struct
        where T4 : struct
    {
        ...
        {
            return new Enumerator(_pool1, _pool2, _pool3, _pool4, _primaryEntities, _primaryCount, _primaryIndex);
        }
        ...
        public struct Enumerator
        {
            ...
            public bool MoveNext()
            {
                while (++_index < _primaryCount)
                {
                    ...
                    if (_primaryIndex == 1)
                    {
                        if (_pool2.Has(entity) && _pool3.Has(entity) && _pool4.Has(entity))
                        {
                            _current = entity;
                            return true;
                        }
                    ...
                    {
                        if (_pool1.Has(entity) && _pool3.Has(entity) && _pool4.Has(entity))
                        {
                            _current = entity;
                            return true;
                        }
                    ...
                    {
                        if (_pool1.Has(entity) && _pool2.Has(entity) && _pool4.Has(entity))
                        {
                            _current = entity;
                            return true;
                        }
                    ...
                    {
                        if (_pool1.Has(entity) && _pool2.Has(entity) && _pool3.Has(entity))
                        {
                            _current = entity;
                            return true;
                        }
